...
export async function buildAnalysisReport(userId: string) {
  const [
    ...
    prisma.studentExamResult.findMany({
      ...
      include: {
        exam: { select: { title: true, type: true, classLevel: true } },
      },
  ...
  const answers = rawAnswers as AnalysisAnswer[];
  const examResults = rawExamResults as ExamResult[];
  const gameScores = rawGameScores as GameScore[];
  const studyLogs = rawStudyLogs as StudyLog[];

  // ---- Genel doğru/yanlış/başarı ----
  const totalAnswered = answers.length;
  const correctCount = answers.filter(
    (a: AnalysisAnswer) => a.isCorrect === true,
  ).length;
  const wrongCount = answers.filter(
    (a: AnalysisAnswer) => a.isCorrect === false,
  ).length;
  const successPercent =
    totalAnswered > 0
      ? Math.round((correctCount / totalAnswered) * 1000) / 10
      : 0;

  // ---- Günlük / haftalık / aylık grafik ----
  const dailyChart = buildDailyChart(
    answers.map((a: AnalysisAnswer) => a.answeredAt),
  );
  ...
  answers.forEach((a: AnalysisAnswer) => {
    const topic = a.question.topic;
    const entry = topicStats.get(topic.id) ?? {
      title: topic.title,
      correct: 0,
      total: 0,
    };
  ...
  const topicList = Array.from(topicStats.entries()).map(
    ([topicId, s]) => ({
      topicId,
      title: s.title,
      successRate:
        s.total > 0 ? Math.round((s.correct / s.total) * 1000) / 10 : 0,
      total: s.total,
    }),
  );
  ...
  const outcomeStats = new Map<
    string,
    { description: string; correct: number; total: number }
  >();
  ...
  answers.forEach((a: AnalysisAnswer) => {
    ...
    const entry = outcomeStats.get(outcome.id) ?? {
  ...
  const outcomeRates = Array.from(outcomeStats.entries()).map(
    ([id, s]) => ({
  ...
  const questionMinutes = studyLogs
    .filter((l: StudyLog) => l.source === "question")
    .reduce((sum: number, l: StudyLog) => sum + l.minutes, 0);
  const avgSecondsPerQuestion =
    totalAnswered > 0
      ? Math.round((questionMinutes * 60) / totalAnswered)
      : 0;
  ...
  const examPerformance = {
    ...
    averageSuccess:
      examResults.length > 0
        ? Math.round(
            (examResults.reduce(
              (sum: number, r: ExamResult) => sum + r.successPercent,
              0,
            ) /
              examResults.length) *
              10,
        ...
        : 0,
    recentResults: examResults.slice(0, 5).map((r: ExamResult) => ({
  ...
  const gamePerformance = {
    ...
    averageScore:
      gameScores.length > 0
        ? Math.round(
            gameScores.reduce(
              (sum: number, g: GameScore) => sum + g.score,
              0,
            ) / gameScores.length,
          )
        : 0,
    bestGame:
      gameScores.length > 0
        ? gameScores.reduce(
            (a: GameScore, b: GameScore) => (a.score > b.score ? a : b),
          )
        : null,
  };
  ...
  const totalStudyMinutes = studyLogs.reduce(
    (sum: number, l: StudyLog) => sum + l.minutes,
    0,
  );
  ...
  const developmentScore = Math.round(
    successPercent * 0.4 +
      examPerformance.averageSuccess * 0.25 +
      streakScore * 0.2 +
      volumeScore * 0.15,
  );
